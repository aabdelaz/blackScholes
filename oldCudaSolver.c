...
void getPv(float *v, int n) {
    cublasStatus_t status;
    int i;
    float *v_h = (float*)malloc(n*sizeof(float));
    status = cublasGetVector(n, sizeof(float), v, 1, v_h, 1);
...
void getPm(float *A, int m, int n) {
    cublasStatus_t status;
    int i, j;
    float *A_h = (float*)malloc(m*n*sizeof(float));
    status = cublasGetVector(m*n, sizeof(float), A, 1, A_h, 1);
    checkCublasStatus(status);
...
void krylov(cublasHandle_t handle, float *w, int m, float l, const float *A, int n, const float *v, int expo) {
    ...
    cudaError_t error;
    cublasStatus_t status;
    float *V = 0, *H = 0, *E = 0, *E_d, *bVe = 0, zero = 0, beta;
    int i, j;

    error = cudaMalloc((void **)&V, n*m*sizeof(V[0]));
    checkError(error);

    // so supposedly setting alpha and beta to 0 sets V to 0's 
    status = cublasSgeam(handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, &zero, NULL, n, &zero, NULL, n, V, n);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "Zeroing out V didn't work: status code is %d\n", status);
        exit(1);
    }

    H = (float *)malloc(m*m*sizeof(H[0]));
    E = (float *)malloc(m*m*sizeof(E[0]));
    if (H == 0 || E == 0) {
    ...
    // is H correct? Mostly, but there seem to be some small floating-point errors. 
 
    // scale H by l
    for (i = 0; i < m*m; i++) H[i] *= l;
    status = cudaMalloc((void**)&bVe, m*n*sizeof(bVe[0]));
    checkCublasStatus(status);

    status = cudaMalloc((void**)&E_d, m*m*sizeof(E_d[0]));
    checkCublasStatus(status);

    if (expo == 1) padeExp(H, E, m);
    else phi(H, E, m);
    ...
    // E is in host memory, so copy that bad boy into E_d
    status = cublasSetVector(m*m, sizeof(E[0]), E, 1, E_d, 1);
    checkCublasStatus(status);

    status = cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, m, &beta, V, n, E_d, m, &zero, bVe, n); 
    checkCublasStatus(status);
